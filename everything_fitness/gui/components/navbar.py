...
class Navbar(Frame):
    ...
    relative_path = 'everything_fitness\\assets\\icons\\'
    ...
    def __init__(self, parent, scenes):
        ...
        self.home_button = Button(self, 
                                  image=self.home_image, 
                                  command=lambda: self.parent.change_page(self.scenes[0]))
        self.home_button.grid(padx=5, pady=5, row=0, column=0)

        self.exercise_button = Button(self, 
                                      image=self.exercise_image, 
                                      command=lambda: self.parent.change_page(self.scenes[1]))
        self.exercise_button.grid(padx=5, pady=5, row=1, column=0)

        self.weight_button = Button(self, 
                                    image=self.weight_image, 
                                    command=lambda: self.parent.change_page(self.scenes[2]))
        self.weight_button.grid(padx=5, pady=5, row=2, column=0)

        self.step_button = Button(self, 
                                  image=self.step_image, 
                                  command=lambda: self.parent.change_page(self.scenes[3]))
        self.step_button.grid(padx=5, pady=5, row=3, column=0)

        self.calorie_button = Button(self, 
                                     image=self.calorie_image, 
                                     command=lambda: self.parent.change_page(self.scenes[4]))
