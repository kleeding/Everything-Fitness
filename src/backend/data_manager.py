...
class DataManager():
    ...
    def add_record(self, date, record, name):
        ...
        if name == "exercise":  ## Remove conditional, keep single for loop
            for i in range(len(self.data)):
                if  date > self.data[i][0]:
                    break
                elif date == self.data[i][0] and record[1] == self.data[i][1]: ## <- a and ((b and c) or c)
                    self.data[i] = record
                    update = True
                    break
                else:
            ...
            for i in range(len(self.data)):
                if date > self.data[i][0]:
                    break
                elif date == self.data[i][0]:
                    self.data[i] = record
                    update = True
                    break
                else:
                    index += 1
        self.data.insert(index, record) if not update else 0
